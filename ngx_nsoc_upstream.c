...
ngx_nsoc_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    *cf = pcf;
    ...
    if (uscf->servers->nelts == 0) {
        ngx_conf_log_error(
                NGX_LOG_EMERG, cf, 0, "no servers are inside upstream");
        return NGX_CONF_ERROR ;
    }
...
ngx_nsoc_upstream_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ngx_parse_url(cf->pool, &u) != NGX_OK) {
        if (u.err) {
            ngx_conf_log_error(
                    NGX_LOG_EMERG, cf, 0, "%s in upstream \"%V\"", u.err,
                    &u.url);
        }
    ...
    if (u.no_port) {
        ngx_conf_log_error(
                NGX_LOG_EMERG, cf, 0, "no port in upstream \"%V\"", &u.url);
        return NGX_CONF_ERROR ;
    }
...
ngx_nsoc_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags)
{
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        if (uscfp[i]->host.len != u->host.len
                || ngx_strncasecmp(
                        uscfp[i]->host.data, u->host.data, u->host.len) != 0) {
            continue;
        }
