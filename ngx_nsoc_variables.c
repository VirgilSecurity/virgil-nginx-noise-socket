...
ngx_nsoc_add_variable(ngx_conf_t *cf, ngx_str_t *name, ngx_uint_t flags)
{
    ...
    if (rc == NGX_BUSY) {
        ngx_conf_log_error(
                NGX_LOG_EMERG, cf, 0, "conflicting variable name \"%V\"", name);
        return NULL;
    }
        ...
        ngx_uint_t index)
{
    ...
    if (cmcf->variables.nelts <= index) {
        ngx_log_error(
                NGX_LOG_ALERT, s->connection->log, 0,
        ...
        ngx_uint_t key)
{
    ...
    for (i = 0; i < cmcf->prefix_variables.nelts; i++) {
        if (name->len >= v[i].name.len && name->len > len
                && ngx_strncmp(name->data, v[i].name.data, v[i].name.len)
                        == 0) {
            len = v[i].name.len;
            n = i;
        }
        ...
        uintptr_t data)
{
    ...
    ngx_memcpy(
            p, ngx_cached_http_log_iso8601.data,
        ...
        ngx_nsoc_regex_t *re, ngx_str_t *str)
{
    ...
    if (rc < 0) {
        ngx_log_error(
                NGX_LOG_ALERT, s->connection->log, 0,
                ngx_regex_exec_n " failed: %i on \"%V\" using \"%V\"", rc, str,
                &re->name);
        return NGX_ERROR;
    }
...
ngx_int_t ngx_nsoc_variables_init_vars(ngx_conf_t *cf)
{
    ...
    for (i = 0; i < cmcf->variables.nelts; i++) {
        ...
        for (n = 0; n < cmcf->variables_keys->keys.nelts; n++) {
            ...
            if (v[i].name.len == key[n].key.len
                    && ngx_strncmp(
                            v[i].name.data, key[n].key.data, v[i].name.len)
                            == 0) {
        ...
        for (n = 0; n < cmcf->prefix_variables.nelts; n++) {
            if (v[i].name.len >= pv[n].name.len && v[i].name.len > len
                    && ngx_strncmp(
                            v[i].name.data, pv[n].name.data, pv[n].name.len)
                            == 0) {
                av = &pv[n];
                len = pv[n].name.len;
            }
        ...
        if (v[i].get_handler == NULL) {
            ngx_log_error(
                    NGX_LOG_EMERG, cf->log, 0, "unknown \"%V\" variable",
                    &v[i].name);
            return NGX_ERROR;
        }
    ...
    if (ngx_hash_init(
            &hash, cmcf->variables_keys->keys.elts,
