        ...
        ngx_chain_t *in, ngx_uint_t from_upstream)
{
    ...
    for (cl = *out; cl; cl = cl->next) {
        ...
        ngx_log_debug7(
                ...
                "write old buf t:%d f:%d %p, pos %p, size: %z "
                        "file: %O, size: %O", cl->buf->temporary,
        ...
        if (ngx_buf_size(cl->buf) == 0 && !ngx_buf_special(cl->buf)) {
            ngx_log_error(
                    NGX_LOG_ALERT, c->log, 0, "zero size buf in writer "
    ...
    *ll = NULL;
    ...
    ngx_log_debug3(
            ...
            "NOISE write filter: l:%ui f:%ui s:%O", last, flush, size);
    ...
    *out = chain;
    ...
    if (chain) {
        if (c->shared) {
            ngx_log_error(
                    NGX_LOG_ALERT, c->log, 0, "shared connection is busy");
            return NGX_ERROR;
        }
