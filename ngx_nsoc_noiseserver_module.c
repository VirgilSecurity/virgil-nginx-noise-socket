...
static ngx_command_t ngx_nsoc_noiseserver_commands[] =
{
  ...
  { ngx_string("client_noise_xor_symb"),
    NGX_NSOC_MAIN_CONF | NGX_NSOC_SRV_CONF | NGX_CONF_TAKE1,
    ngx_nsoc_conf_set_char_slot,
    NGX_NSOC_SRV_CONF_OFFSET,
    offsetof(ngx_nsoc_noiseserver_conf_t, xor_symb),
    NULL },
...
ngx_nsoc_noiseserver_create_conf(ngx_conf_t *cf)
{
    ...
    noisecf->xor_symb = NGX_CONF_UNSET;
        ...
        void *child)
{
    ...
    ngx_conf_merge_value(
            conf->prefer_server_ciphers, prev->prefer_server_ciphers, 0);
    ...
    conf->noise->ctx->XOR_symb = conf->xor_symb;
